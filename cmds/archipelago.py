...
class Archipelago(commands.GroupCog, group_name="archipelago"):
    ...
    async def archivist_log(
        self, interaction: discord.Interaction, type: str, message: str
    ):
        """Log an action to the archivist log channel, if set."""

        if interaction.guild.id != 1424283904260706378:
            logger.error(
                "Attempted to log to archivist channel outside of main server."
            )
            return False
    ...
    db = app_commands.Group(
        name="db", description="Query the bot's Archipelago database"
    )

    # First some helpers
    async def db_table_complete(
    ...
    async def db_game_complete(
        self, ctx: discord.Interaction, current: str
    ) -> typing.List[app_commands.Choice[str]]:
        cursor = sqlcon.cursor()
        cursor.execute(
            "select game, count(*) from archipelago.item_classifications group by game;"
        )
    ...
    async def db_item_complete(
        ...
        if len(current) == 0:
            ...
            return [
                app_commands.Choice(name=f"{current} (Multi-Selection)", value=current)
            ]
        else:
            return [
                app_commands.Choice(name=opt, value=opt)
    ...
    async def db_group_complete(
        ...
        if len(response) == 0:
            return [app_commands.Choice(name="There are no item groups for this game.", value="No Groups")]
        elif len(current) == 0:
    ...
    async def db_location_complete(
        ...
        game_selection = ctx.data["options"][0]["options"][0]["options"][0]["value"]
        cursor.execute(
            f"select location from archipelago.game_locations where game = '{str(game_selection)}';"
        )
        response = sorted([opt[0] for opt in cursor.fetchall()])
        if len(current) == 0:
            return [app_commands.Choice(name=opt, value=opt) for opt in response[:20]]
    ...
    async def db_classification_complete(
        self, ctx: discord.Interaction, current: str
    ) -> typing.List[app_commands.Choice[str]]:
        permitted_values = [
        ...
        if len(current) == 0:
            return [
                app_commands.Choice(name=opt.title(), value=opt)
                for opt in permitted_values
            ]
        else:
            return [
                app_commands.Choice(name=opt.title(), value=opt)
    ...
    async def db_select(
        ...
        cursor = sqlcon.cursor()
        logger.info(
            f"executed SQL command from discord: SELECT {selection} FROM {table} {f'WHERE {where}' if bool(where) else ''};"
        )
        cursor.execute(
            f"SELECT {selection} FROM {table} {f'WHERE {where}' if bool(where) else ''};"
        )
        response = cursor.fetchall()
        ...
        try:
            ...
            await interaction.response.send_message(
                "Here's the result, as a file:",
                file=discord.File(BytesIO(responsefile), "result.txt"),
    ...
    async def db_update_item_classification(
        self,
        interaction: discord.Interaction,
        game: str,
        classification: str,
        ...
        if "%" in item or r"?" in item:
            ...
            matched_items = [r[0] for r in cursor.fetchall()]
            logger.info(
                f"Classified {str(count)} item(s) matching '{item}' in {game} to {classification}"
            )

            await self.archivist_log(
                interaction,
                "classify",
                f"Classified **{join_words(matched_items)}** in **{game}** to **{classification.title()}**.",
            )
            ...
            try:
                ...
                for port in ports:
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Failed to contact itemlog at port {port} to refresh classifications for {game}:{item}."
                        )

                await self.archivist_log(
                    interaction,
                    "classify",
                    f"Classified **{item}** in **{game}** to **{classification.title()}**.",
                )
    ...
    @app_commands.describe(
        game="The game that contains the item", item="The item to act on"
    )
    @app_commands.autocomplete(game=db_game_complete, item=db_item_complete)
    async def db_set_item_description(
        self, interaction: discord.Interaction, game: str, item: str
    ):
        ...
        cursor.execute(
            "SELECT description FROM archipelago.item_classifications WHERE game = %s AND item = %s",
            (game, item),
        )
        ...
        class DescriptionForm(discord.ui.Modal):
            ...
            def __init__(self, cogself, game: str, item: str):
                super().__init__(
                    title=f"{item} ({game})"[:45]
                )  # Title must be under 45 characters
            ...
            description = discord.ui.TextInput(
                ...
                placeholder=existing_description[:96] + "..."
                if bool(existing_description)
                else f"Enter the description for {item} here.",
                required=True,
                max_length=500,
            )
            ...
            async def on_submit(self, interaction: discord.Interaction):
                ...
                cursor.execute(
                    "UPDATE archipelago.item_classifications SET description = %s WHERE game = %s AND item = %s",
                    (description, self.game, self.item),
                )
                await interaction.response.send_message(
                    f"Description for {self.game}'s '{self.item}' has been set.",
                    ephemeral=True,
                )
                await self.cogself.archivist_log(
                    interaction,
                    "describe",
                    f"Set description for **{self.item}** in **{self.game}**.",
                )
                logger.info(
                    f"User {interaction.user.display_name} ({interaction.user.id}) set description for {self.game}'s {self.item}."
                )
    ...
    async def import_datapackage(
        self,
        interaction: discord.Interaction,
        url: str = "https://archipelago.gg/datapackage",
        export_json: discord.Attachment = None,
    ):
        ...
        with sqlcon.cursor() as cursor:
            ...
            if export_json:
                try:  # Make sure it's actually json
                    if export_json.content_type != "application/json; charset=utf-8":
                        logger.error(
                            f"Import datapackage: provided file has invalid content type {export_json.content_type}"
                        )
                        return await newpost.edit(
                            content="**Error**: the provided file is not valid JSON.",
                            delete_after=15.0,
                        )
                    ...
                    return await newpost.edit(
                        content=f"**Error**: {e}", delete_after=15.0
                    )
            else:
            ...
            for game, data in datapackage["games"].items():
                ...
                if "Archipelago" in games_list:
                    games_list.remove("Archipelago")
                current_index = games_list.index(game)
                next_game = (
                    games_list[current_index + 1]
                    if current_index + 1 < len(games_list)
                    else None
                )
                ...
                if cursor.fetchone():
                    logger.info(
                        f"Datapackage for {game} with checksum {checksum} is already imported; skipping."
                    )
                    if next_game:
                        await newpost.edit(
                            content=f"Skipped {game} (already imported), working on {next_game}..."
                        )
                    else:
                        pass
                    continue
                ...
                if next_game:
                    await newpost.edit(
                        content=f"Imported {game}, working on {next_game}..."
                    )
                else:
                    pass
    ...
    async def cleanup_fake_items(self, interaction: discord.Interaction):
        ...
        with sqlcon.cursor() as cursor:
            ...
            cursor.execute(
                "DELETE FROM archipelago.item_classifications WHERE datapackage_checksum IS NULL OR datapackage_checksum = ''"
            )
            deleted_count = cursor.rowcount
            sqlcon.commit()

            await newpost.edit(
                content=f"Cleaned up {deleted_count} fake items from the database."
            )
    ...
    @app_commands.describe(
        game="The game to import classifications for (omit to import all)",
        skip_classified="Skip items that already have a classification",
    )
    async def import_classifications(
        ...
        def fetch_classifications(game: str):
            ...
            community_progression = requests.get(
                f"https://raw.githubusercontent.com/silasary/world_data/refs/heads/main/worlds/{game}/progression.txt"
            )
            if community_progression.status_code == 200:
                ...
                logger.info(
                    f"Retrieved community classifications for {game} from world_data repository."
                )
        ...
        if not bool(game):
            with sqlcon.cursor() as cursor:
                cursor.execute(
                    "SELECT DISTINCT game FROM archipelago.item_classifications;"
                )
            ...
            for game, classifications in comm_classification_table.items():
                for item, classification in classifications.items():
                    if classification not in [
                        ...
                        logger.warning(
                            f"Invalid classification '{classification}' for {game}: {item}. Skipping."
                        )
                        skipped += 1
                        continue
        ...
        return await newpost.edit(
            content=f"Import of community classifications complete! Processed {processed} items, updated {updated} items, skipped {skipped} items (bad classifications)."
        )
    ...
    async def export_classifications(self, interaction: discord.Interaction, game: str):
        ...
        with sqlcon.cursor() as cursor:
            cursor.execute(
                "SELECT item, classification FROM archipelago.item_classifications WHERE game = %s and classification IS NOT NULL ORDER BY item asc;",
                (game,),
            )
        ...
        response = "\n".join(
            [
                ...
                for item, classification in export_data.items()
            ]
        )
    ...
    async def db_permissions_error(
        self, interaction: discord.Interaction, error: app_commands.AppCommandError
    ):
        if isinstance(error, app_commands.MissingRole):
            return await interaction.response.send_message(
                "You need the `archivist` role in order to manipulate the bot's Archipelago database.",
                ephemeral=True,
            )
        else:
            return await interaction.response.send_message(
                f"Database command error: {error}", ephemeral=True
            )

    aproom = app_commands.Group(
        name="room", description="Commands to do with the current room"
    )

    async def link_slot_unlinked_complete(
        self, ctx: discord.Interaction, current: str
    ) -> typing.List[app_commands.Choice[str]]:
    ...
    async def link_slot_complete(
        ...
        if len(current) == 0:
            return [
                app_commands.Choice(name=opt, value=opt) for opt in permitted_values
            ]
        else:
            return [
                app_commands.Choice(name=opt, value=opt)
    ...
    async def user_linked_slots_complete(
        self, ctx: discord.Interaction, current: str
    ) -> typing.List[app_commands.Choice[str]]:
    ...
    async def link_slot(self, interaction: discord.Interaction, slot_name: str):
        ...
        logger.info(
            f"Linked {slot_name} to {user.display_name} ({user.id}) in {interaction.guild.name} ({interaction.guild.id})"
        )
        return await interaction.response.send_message(
            f"Linked **{slot_name}** to **{user.display_name}**!", ephemeral=True
        )
    ...
    async def setroom(self, interaction: discord.Interaction, room_url: str):
        ...
        logger.info(
            f"Setting room for {interaction.guild.name} ({interaction.guild.id}) to {room_url}..."
        )
        ...
        if room_url.split("/")[-2] != "room":
            return await newpost.edit(
                content="**Error**: the provided URL is not an Archipelago room URL.",
                delete_after=15.0,
            )
        ...
        try:
            ...
            return await newpost.edit(
                content="**Error**: the provided URL is not responding. Please check the URL and try again.",
                delete_after=15.0,
            )
        except requests.exceptions.RequestException as e:
            logger.error(f"Error fetching room data: {e}")
            return await newpost.edit(
                content="**Error**: there was a problem fetching the room data. Please try again later.",
                delete_after=15.0,
            )
        ...
        with sqlcon.cursor() as cursor:
            ...
            for command in commands:
                ...
                try:
                    ...
                    await newpost.edit(
                        content=f"**Error**: there was a problem executing the SQL command. Please try again later.\n\n```{e}```"
                    )
                    return
        ...
        logger.info(
            f"Set room for {interaction.guild.name} ({interaction.guild.id}) to {room_url}"
        )
        await newpost.edit(
            content=f"Set room for {interaction.guild.name} to {room_url} !"
        )
    ...
    async def room_status(
        ...
        if not self.ctx.extras.get("ap_rooms"):
            ...
            if not self.ctx.extras["ap_rooms"].get(interaction.guild_id):
                return await newpost.edit(
                    content="No Archipelago room is currently set for this server."
                )
        ...
        if not api_port:
            ...
            if not api_port:
                return await newpost.edit(
                    content="Something is wrong, and the API port found for this room is invalid. Contact Splatsune."
                )

        try:
            game_table = requests.get(
                f"http://localhost:{api_port}/inspectgame", timeout=10
            ).json()
        ...
        def player_status(player: dict) -> list[str]:
            ...
            if player["goaled"] is True:
                ...
                status_lines.append(
                    f"- **{player['name']}{showgame_ifenabled(player)}**: released from the game."
                )
            else:
                status_lines.append(
                    f"- **{player['name']}{showgame_ifenabled(player)}**: {round(player['collection_percentage'], 1)}% complete. ({player['collected_locations']}/{player['total_locations']} checks.) {last_online(player)}"
                )
        ...
        with sqlcon.cursor() as cursor:
            try:
                ...
                msg_lines.append(
                    f"**Room ID** [{room_id}](<https://{host}/room/{room_id}>) (`{host}:{port}`)"
                )
            except psql.Error as e:
                pass

        msg_lines.append(
            f"This game is {round(game_table['collection_percentage'], 2)}% complete. ({game_table['collected_locations']} out of {game_table['total_locations']} locations checked.)"
        )
        if game_table["running"] is False:
            msg_lines.append(
                "The game is currently spun down - visit the room page to bring it back up."
            )
        ...
        linked_player_list = {
            k: v for k, v in game_table["players"].items() if v["name"] in linked_slots
        }
        other_players_list = {
            k: v
            for k, v in game_table["players"].items()
            if v["name"] not in linked_slots
        }
        if filter_active:
            other_players_list = {
                k: v
                for k, v in other_players_list.items()
                if not v["goaled"] and not v["released"]
            }
        ...
        if not filter_self:
            ...
            sorted_other_players_list = iter(
                sorted(
                    other_players_list.items(),
                    key=lambda p: (
                        not p[1]["online"],
                        -int(p[1].get("last_online") or 0),
                    ),
                )
            )
            ...
            while len("\n".join(msg_lines)) < 1900:
                try:
                    ...
                    if len("\n".join(msg_lines)) + len("\n".join(new_lines)) > 1800:
                        msg_lines.append(
                            "- ...and more players not shown to avoid message length limits."
                        )
                        break
                    else:
    ...
    @app_commands.describe(
        minimum_importance="Minimum importance of items to show",
        include_unclassified="Include unclassified items",
    )
    async def received_items(
        self,
        interaction: discord.Interaction,
        minimum_importance: str = "useful",
        ...
        for slot in linked_slots:
            ...
            player_last_online = (
                player["last_online"] if player["last_online"] is not None else 0
            )
            ...
            for item in player["inventory"]:
                try:
                    if (
                        item.get("received_timestamp", 0) > player_last_online
                        and item["classification"] in show_classifications
                    ):
                    ...
                    if (
                        player_last_online is None
                        and player_table[slot]["online"] is True
                    ):
                        ...
                        item["received_timestamp"] is None
                        and player_last_online is None
                    ):
                        ...
                        logger.warning(
                            f"Received item for {slot} has invalid timestamp data: {item['name']} - {item.get('received_timestamp', 'None')} vs {player_last_online}"
                        )
                        continue

        if all(len(player_table[slot]["offline_items"]) == 0 for slot in linked_slots):
            return await newpost.edit(
                content="You have not received any items since you last played."
            )
        ...
        while (len("\n".join(rcv_lines)) + msglen < 1800) and not all(
            exhausted.values()
        ):
        ...
        for slot in linked_slots:
            ...
            item_lines[slot] = sorted(
                item_lines[slot].copy(), key=lambda ts: ts.split(" ")[1], reverse=True
            )
        ...
        logger.info(
            f"Built received list of {sum(len(lines) for lines in item_lines.values())} items for {len(linked_slots)} slots in {iteration} iterations. Length: {msglen} chars."
        )
    ...
    async def gethints(self, interaction: discord.Interaction, public: bool = False):
        ...
        room_slots = requests.get(
            f"https://{room['host']}/api/room_status/{room['room_id']}", timeout=10
        ).json()["players"]
        ...
        for slot in linked_slots:
            if slot in game_table["players"]:
                if (
                    game_table["players"][slot]["goaled"]
                    or game_table["players"][slot]["released"]
                ):
                    continue
                hint_table[slot] = {}
                for item in game_table["players"][slot]["hints"]["sending"]:
                    if item["found"] is True:
                        continue
                    if item["classification"] in ["trap", "filler", "currency"]:
                        continue
                    if any(
                        [
                            game_table["players"][item["receiver"]]["released"],
                            game_table["players"][item["receiver"]]["goaled"],
                        ]
                    ):
                        continue
                ...
                for item in game_table["players"][slot]["hints"]["receiving"]:
                    ...
                    if item["classification"] in ["trap", "filler", "currency"]:
                        continue
                    if item["location"]["player"] in linked_slots:
                        continue
                    if any(
                        [
                            game_table["players"][item["receiver"]]["released"],
        ...
        if len(hint_table_list) == 0:
            return await newpost.edit(
                content="No hints available for your linked slots."
            )

        hints_list = "## To Find:"
        for hint in hint_table_list:
            if hint["Sender"] not in linked_slots:
                continue
            if (
                game_table["players"][hint["Receiver"]]["goaled"] is True
                or game_table["players"][hint["Receiver"]]["released"] is True
            ):
                continue
        ...
        hints_list += "\n\n## To Be Found:"
        for hint in hint_table_list:
            if hint["Receiver"] not in linked_slots:
                continue
            if hint["Sender"] == hint["Receiver"]:
                continue
    ...
    @app_commands.describe(
        slot="Linked slot to upload to",
        slot_file="File to upload. Run this command without for more info.",
    )
    async def upload_data(
        self,
        interaction: discord.Interaction,
        slot: str,
        slot_file: discord.Attachment = None,
    ):
        ...
        if not game_table:
            return await newpost.edit(
                content="Couldn't fetch the game table from the running Archipelago game."
            )
        ...
        if slot_file is None:
            ...
            match game_table["players"][slot]["game"]:
                case "Trackmania":
                    ...
                    up_request = requests.post(
                        f"http://localhost:{api_port}/upload_data/{slot}",
                        json=upload_json,
                    ...
                    if up_request.status_code == 200:
                        ...
                        return await newpost.edit(
                            content=f"✗ {up_request.status_code}: {up_request.text}"
                        )
                case _:
                    # Not supported
                    return await newpost.edit(
                        content="Sorry, I can't use that file - your game doesn't support/need this."
                    )
    ...
    def fetch_guild_room(self, guild_id: int) -> dict:
        ...
        if room and room.get("last_activity") and room.get("port"):
            ...
            with sqlcon.cursor() as cursor:
                cursor.execute(
                    "SELECT * FROM pepper.ap_all_rooms WHERE guild = %s and active = 'true' LIMIT 1",
                    (guild_id,),
                )
                result = cursor.fetchone()
                if result:
    ...
    async def on_ready(self):
        ...
        if len(cfg["bot"]["archipelago"]["itemlogs"]) > 0:
            ...
            for log in cfg["bot"]["archipelago"]["itemlogs"]:
                ...
                env["WEBHOOK_URL"] = (
                    log["webhooks"][0] if len(log["webhooks"]) > 0 else None
                )
                ...
                try:
                    script_path = os.path.join(
                        os.path.dirname(__file__), "..", "ap_itemlog.py"
                    )
